...
function Page() {
  ...
  }, [grossPayroll, trainingExpenses, apprenticeshipExpenses, employeeCount, year, semester, isApprenticeshipGapJustified]);
    ...
    <div className="space-y-8">
      ...
      <header className="flex flex-col gap-2">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
            ...
            <span className={`h-1.5 w-1.5 rounded-full ${result.isSubjectToTax ? "bg-emerald-500" : "bg-amber-500"}`} />
      ...
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <section className="surface-card space-y-4 lg:col-span-5 h-fit">
          <h2 className="title-text">{locale === "ar" ? "معطيات التصريح" : locale === "en" ? "Declaration parameters" : "Paramètres de la déclaration"}</h2>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select 
                ...
                className="w-full rounded-lg border border-input bg-surface px-3 py-2 focus:border-primary focus:outline-none"
              >
                <option value={1}>{locale === "ar" ? "السداسي 1" : locale === "en" ? "Semester 1" : "Semestre 1"}</option>
                <option value={2}>{locale === "ar" ? "السداسي 2" : locale === "en" ? "Semester 2" : "Semestre 2"}</option>
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="label-text mb-1 block">{locale === "ar" ? "الهدف القانوني" : locale === "en" ? "Legal Target" : "Taux cible"}</label>
          ...
          {result.isSubjectToTax && (
            <div className="flex items-start gap-2 pt-2 border-t border-border">
              ...
              />
              <label htmlFor="gapJustified" className="text-xs text-ink-muted cursor-pointer leading-tight">
                {t("tfpc.justified")}
              </label>
            </div>
          )}
        ...
        <section className="space-y-6 lg:col-span-7">
            ...
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-sm text-amber-500 flex items-start gap-3">
              ...
              <div>
                <strong className="font-semibold block mb-1">
                  {locale === "ar" ? "إعفاء قانوني" : locale === "en" ? "Legal Exemption" : "Exonération légale"}
                </strong>
                {locale === "ar" 
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="surface-card space-y-4">
              <div className="border-b border-border pb-2">
                <h3 className="font-semibold text-primary text-sm tracking-wide uppercase">
                  {locale === "ar" ? "1- رسم التكوين المتواصل" : locale === "en" ? "1- Training Tax (TFPC)" : "1- Taxe Formation (TFPC)"}
                </h3>
              </div>
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  <span className="text-ink-muted">{locale === "ar" ? "الهدف (1%)" : locale === "en" ? "Target (1%)" : "Cible (1%)"}</span>
                  <span className="tabular-nums font-medium">{formatCurrency(result.tfpcTarget, locale)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">{locale === "ar" ? "النفقات المنجزة" : locale === "en" ? "Expenses" : "Dépenses réalisées"}</span>
                  <span className="tabular-nums text-emerald-500 font-medium">− {formatCurrency(result.trainingExpenses, locale)}</span>
                </div>
                <div className="flex justify-between border-t border-border/50 pt-2">
                  <span className="text-ink-muted">{locale === "ar" ? "نسبة الجهد" : locale === "en" ? "Effort Ratio" : "Ratio d'effort"}</span>
                  <span className="tabular-nums font-medium">{formatPercent(result.trainingRatio, locale, 2)} / 1.00%</span>
                </div>
                <div className="flex justify-between border-t border-border/50 pt-2 font-semibold text-ink">
                  <span>{t("tfpc.tfpcDue")}</span>
                  <span className="tabular-nums text-primary">{formatCurrency(result.tfpcDue, locale)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="surface-card space-y-4">
              <div className="border-b border-border pb-2">
                <h3 className="font-semibold text-primary text-sm tracking-wide uppercase">
                  {locale === "ar" ? "2- رسم التمهين" : locale === "en" ? "2- Apprenticeship Tax (TA)" : "2- Taxe Apprentissage (TA)"}
                </h3>
              </div>
              <div className="space-y-2.5 text-sm">
                <div className="flex justify-between">
                  <span className="text-ink-muted">{locale === "ar" ? "الهدف (1%)" : locale === "en" ? "Target (1%)" : "Cible (1%)"}</span>
                  <span className="tabular-nums font-medium">{formatCurrency(result.taTarget, locale)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-ink-muted">{locale === "ar" ? "النفقات المنجزة" : locale === "en" ? "Expenses" : "Dépenses réalisées"}</span>
                  <span className="tabular-nums text-emerald-500 font-medium">− {formatCurrency(result.apprenticeshipExpenses, locale)}</span>
                </div>
                {result.apprenticeshipOffsetApplied > 0 && (
                  <div className="flex justify-between text-xs text-indigo-500">
                    ...
                    <span className="tabular-nums">− {formatCurrency(result.apprenticeshipOffsetApplied, locale)}</span>
                  </div>
                )}
                <div className="flex justify-between border-t border-border/50 pt-2">
                  <span className="text-ink-muted">{locale === "ar" ? "نسبة الجهد" : locale === "en" ? "Effort Ratio" : "Ratio d'effort"}</span>
                  <span className="tabular-nums font-medium">{formatPercent(result.apprenticeshipRatio, locale, 2)} / 1.00%</span>
                </div>
                <div className="flex justify-between border-t border-border/50 pt-2 font-semibold text-ink">
                  <span>{t("tfpc.taDue")}</span>
                  <span className="tabular-nums text-primary">{formatCurrency(result.taDue, locale)}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="surface-card bg-primary/[0.03] border-primary/20 space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <span className="label-text text-sm uppercase tracking-wider">{locale === "ar" ? "إجمالي الرسوم المستحقة" : locale === "en" ? "Total taxes due" : "Total des taxes dues"}</span>
                <div className="mt-2 text-4xl font-bold tabular-nums text-primary">
                  {formatCurrency(result.totalDue, locale)}
                </div>
              </div>
              <div className="text-right hidden sm:block">
                <span className="text-xs text-ink-muted block">{locale === "ar" ? "تاريخ الاستحقاق" : locale === "en" ? "Due Date" : "Échéance"}</span>
      ...
      <section className="surface-card bg-muted/40 border border-border">
        <h2 className="title-text text-sm mb-3 uppercase tracking-wider">{locale === "ar" ? "مراجع قانونية - قانون المالية 2026" : locale === "en" ? "Legal references - Finance Act 2026" : "Références légales - Loi de Finances 2026"}</h2>
        <div className="grid gap-4 md:grid-cols-2 text-xs text-ink-muted">
          <div className="space-y-2">
            <p className="font-semibold text-ink">Art. 196 bis & quater CIDTA (Art. 18-19 LF 2026)</p>
            <p>
              {locale === "ar"
          ...
          <div className="space-y-2">
            <p className="font-semibold text-ink">Art. 196 quinquies & sexies CIDTA (Art. 20-21 LF 2026)</p>
            <p>
              {locale === "ar"
