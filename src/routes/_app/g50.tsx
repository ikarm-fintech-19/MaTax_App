...
interface DraftLine extends OperationLineInput { id: string }

function loadDraft(): { lines: DraftLine[]; deductions: DeductionsInput; period: G50Input["period"] } | null {
...
function G50Page() {
  ...
  const [period, setPeriod] = useState<G50Input["period"]>({
    kind: "monthly", year: now.getFullYear(), month: now.getMonth() + 1,
  });
  const [lines, setLines] = useState<DraftLine[]>([
    { id: crypto.randomUUID(), code: "E3B8", caHT: 0 },
  ]);
  ...
  useEffect(() => {
    ...
    if (draft) {
      setLines(draft.lines.length ? draft.lines : [{ id: crypto.randomUUID(), code: "E3B8", caHT: 0 }]);
  ...
  const addLine = () =>
    setLines((prev) => [...prev, { id: crypto.randomUUID(), code: "E3B8", caHT: 0 }]);
  ...
  const handleExport = () => {
    ...
    const periodName = input.period.kind === "monthly" && input.period.month
    ...
    <div className="space-y-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <Download size={16} /> {t("common.export_pdf")}
          </button>
        </div>
      </header>
      ...
      <section className="space-y-3">
        ...
        <div className="surface-card grid grid-cols-1 gap-4 md:grid-cols-2">
          <Field label="Raison sociale" value={company.raisonSociale}
            onChange={(v) => saveCompany({ ...company, raisonSociale: v })} />
          <Field label="NIF" value={company.nif}
            onChange={(v) => saveCompany({ ...company, nif: v.replace(/\D/g, "").slice(0, 20) })} />
          <Field label="Activité / Profession" value={company.activite}
            onChange={(v) => saveCompany({ ...company, activite: v })} />
          <Field label="Code activité" value={company.codeActivite ?? ""}
            onChange={(v) => saveCompany({ ...company, codeActivite: v })} />
          <Field label="Article d'imposition" value={company.articleImposition ?? ""}
        ...
        <div className="surface-card flex flex-wrap items-end gap-3">
          <div className="inline-flex rounded-lg border border-border p-1">
            {(["monthly", "quarterly"] as const).map((k) => (
              <button key={k}
                onClick={() => setPeriod({ ...period, kind: k, month: k === "monthly" ? (period.month ?? 1) : undefined, quarter: k === "quarterly" ? (period.quarter ?? 1) : undefined })}
      ...
      <section className="space-y-4">
        <SectionHeader letter="A" title="Chiffres d'affaires imposables" subtitle="Sélectionnez le code officiel (E3B…) puis saisissez le CA HT." />
        ...
        <div className="space-y-3">
          {lines.map((l) => {
            ...
            return (
              <div key={l.id} className="rounded-lg border border-border p-3 sm:border-0 sm:p-0 sm:grid sm:grid-cols-12 sm:items-center sm:gap-2">
                <select value={l.code}
                  ...
                  {G50_OPERATION_LINES.map((d) => (
                    <option key={d.code} value={d.code}>
                      {d.code} — {d.label} {d.kind === "exonere" ? "(exonéré)" : `(${(d.rate * 100).toFixed(0)}%)`}
                    </option>
                  ))}
                </select>
                <div className="mt-2 flex items-center gap-2 sm:col-span-5 sm:mt-0 sm:contents">
                  <input type="number" min={0} value={l.caHT || ""}
                    onChange={(e) => {
                      const v = Number(e.target.value);
                      if (v < 0) { toast.error(t("g50.error_negative")); return; }
                      updateLine(l.id, { caHT: v });
                    }}
                    placeholder="CA HT" className="flex-1 rounded-lg border border-input bg-surface px-3 py-2 text-sm tabular-nums sm:col-span-3" />
                  <div className="min-w-[5rem] text-right text-xs tabular-nums text-ink-muted sm:col-span-1">{formatCurrency(tva, locale)}</div>
                  <button onClick={() => removeLine(l.id)} aria-label="Remove"
                    className="rounded-lg p-2 text-ink-muted hover:text-destructive sm:col-span-1"><Trash2 size={16} /></button>
                </div>
              </div>
            );
          })}
      ...
      <section className="space-y-4">
        ...
        <DeductionField code="E3B90" label="Précompte antérieur"
          value={deductions.precompteAnterieur} onChange={(v) => setDeductions({ ...deductions, precompteAnterieur: v })} />
        <DeductionField code="E3B91" label="TVA sur achats biens/services (art. 29)"
          value={deductions.tvaAchatsBiensServices} onChange={(v) => setDeductions({ ...deductions, tvaAchatsBiensServices: v })} />
        <DeductionField code="E3B92" label="TVA sur achat de biens (art. 38)"
          value={deductions.tvaAchatsBiens} onChange={(v) => setDeductions({ ...deductions, tvaAchatsBiens: v })} />
        <DeductionField code="E3B93" label="Régularisation prorata (déduction complémentaire)"
          value={deductions.proRataDeductionComplementaire} onChange={(v) => setDeductions({ ...deductions, proRataDeductionComplementaire: v })} />
        <DeductionField code="E3B94" label="TVA factures annulées/impayées (art. 18)"
          value={deductions.tvaFacturesAnnulees} onChange={(v) => setDeductions({ ...deductions, tvaFacturesAnnulees: v })} />
        <DeductionField code="E3B95" label="Autres déductions"
          value={deductions.autresDeductions} onChange={(v) => setDeductions({ ...deductions, autresDeductions: v })} />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        {result.byRate.map((b) => (
          <Row key={b.rate} label={`TVA collectée @ ${(b.rate * 100).toFixed(0)}%`} value={formatCurrency(b.tva, locale)} />
        ))}
        <Row label="E3B96 — Total des droits dus" value={formatCurrency(result.totalDroitsDus, locale)} />
        <Row label="E3B110 − Total déductions (B)" value={`− ${formatCurrency(result.totalDeductions, locale)}`} />
        ...
        {result.precompteAReporter > 0 ? (
          <div className="rounded-2xl border border-primary/30 bg-primary/5 p-6 shadow-sm">
            <div className="label-text text-sm uppercase tracking-wider text-primary">{t("g50.credit_title")}</div>
            ...
            <p className="mt-2 font-mono text-[10px] text-primary/70">E3B130 — Précompte à reporter</p>
          </div>
        ) : (
          <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
            <div className="label-text text-sm uppercase tracking-wider">{t("g50.to_pay_title")}</div>
            ...
            <p className="mt-1 font-mono text-[10px] text-ink-muted">
              ...
              {result.tvaAutoLiquidee > 0 ? ` + E3B140 = ${formatCurrency(result.tvaAutoLiquidee, locale)}` : ""}
            </p>
          </div>
        )}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-surface/95 p-4 backdrop-blur md:hidden shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
        <div className="flex items-center justify-between mx-auto max-w-6xl">
          ...
          <div className={`text-xl font-bold tabular-nums ${result.precompteAReporter > 0 ? "text-primary" : ""}`}>
            {formatCurrency(result.precompteAReporter > 0 ? result.precompteAReporter : result.totalTvaAPayer, locale)}
          </div>
        </div>
      </div>
    </div>
...
function SectionHeader({ letter, title, subtitle }: { letter?: string; title: string; subtitle?: string }) {
  return (
    <div className="mb-2">
      ...
      {subtitle && (
        <p className={`mt-2 text-sm text-ink-muted ${letter ? "ms-11" : ""}`}>
          {subtitle}
        </p>
      )}
    </div>
...
function Field({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <div className="block">
      <label htmlFor={id} className="label-text mb-1 block text-xs">{label}</label>
...
function DeductionField({ code, label, value, onChange }: { code: string; label: string; value: number; onChange: (v: number) => void }) {
  return (
