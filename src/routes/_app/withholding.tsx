...
import { calculateWithholding, WITHHOLDING_LABELS, WITHHOLDING_RATES, type WithholdingKind, type WithholdingLine } from "@/lib/engines/withholding";
    ...
    <div className="space-y-8">
      ...
      <section className="surface-card">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button onClick={() => setLines([...lines, { kind: "dividends_resident_individual", base: 0 }])}
        ...
        <div className="space-y-2">
          {lines.map((l, i) => (
            <div key={i} className="grid grid-cols-12 gap-2">
              <select value={l.kind}
                onChange={(e) => setLines(lines.map((x, j) => j === i ? { ...x, kind: e.target.value as WithholdingKind } : x))}
                className="col-span-7 rounded-lg border border-input bg-surface px-3 py-2 text-sm">
                {kinds.map((k) => <option key={k} value={k}>{WITHHOLDING_LABELS[k][locale]} — {formatPercent(WITHHOLDING_RATES[k], locale, 0)}</option>)}
              </select>
              <input type="number" min={0} value={l.base}
                onChange={(e) => setLines(lines.map((x, j) => j === i ? { ...x, base: Number(e.target.value) } : x))}
              ...
              <button onClick={() => setLines(lines.filter((_, j) => j !== i))}
                className="col-span-1 rounded-lg text-ink-muted hover:text-destructive"><Trash2 size={16} className="mx-auto" /></button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="surface-card space-y-2">
        <h2 className="title-text mb-2">{t("common.summary")}</h2>
        ...
        <div className="flex justify-between">
          ...
          <span className="title-text tabular-nums">{formatCurrency(result.totalWithheld, locale)}</span>
        </div>
      </section>
    </div>
  );
